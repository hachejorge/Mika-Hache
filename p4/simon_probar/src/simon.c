...
void simon_mostrar_seq_inicial(EVENTO_T evento, uint32_t aux){
		if(aux < LEDS_NUMBER * 2){
			...
			svc_alarma_activar(0, 250, ev_JUEGO_SEQ_INCIAL, aux + 1);
		}
		else{
				svc_alarma_activar(0,250,ev_JUEGO_CONTINUAR,0);
		}
}
...
void simon_mostrar_seq_juego(EVENTO_T evento, uint32_t aux){
		if(aux < posicion_simon * 2){
			...
			svc_alarma_activar(0, 20 * velocidad, ev_JUEGO_SEQ_JUEGO, aux + 1);
		}
}
...
void simon_conmutar_leds(EVENTO_T evento, uint32_t aux){
		if(aux < 2){
			...
			svc_alarma_activar(0,500, ev_LEDS_ENCENDER, aux + 1);
		}
		else{
			svc_alarma_activar(0, 10, ev_JUEGO_CONTINUAR, 0);
		}
}

void encender_led(EVENTO_T evento, uint32_t aux){
			drv_led_encender(aux);
			svc_alarma_activar(0, 200, ev_LEDS_APAGAR, aux);
}

void apagar_led(EVENTO_T evento, uint32_t aux){
			drv_led_apagar(aux);
			svc_alarma_activar(0, 10, ev_LEDS_SIGUINTE, aux);
}
void reiniciar_juego(EVENTO_T evento, uint32_t aux){
			estado_simon = e_INIT;
			svc_alarma_activar(1,5000,ev_TIEMPO_SUPERADO,0);
			svc_alarma_activar(0,10,ev_INICIAR_JUEGO,0);
...
void simon_iniciar(){
	...
	svc_GE_suscribir(ev_LEDS_APAGAR, apagar_led);
	//svc_GE_suscribir(ev_LEDS_SIGUINTE, simon_tratar);
	...
	svc_alarma_activar(0,100,ev_INICIAR_JUEGO,0);
	...
	switch(estado_simon) 
	{
		case e_INIT:
			if(evento == ev_INICIAR_JUEGO){
				...
				velocidad = 20;
				svc_alarma_activar(0, 10, ev_JUEGO_SEQ_INCIAL, 0);				
		...
		case e_SHOW_SEQUENCE:
			...
			svc_alarma_activar(0, 10, ev_JUEGO_SEQ_JUEGO, 0);
			svc_alarma_activar(0,5000,ev_TIEMPO_SUPERADO,0); // si l jugador tarda mas de 5 s el juego comienza de nuevo
		...
		case e_WAIT_FOR_INPUT:
				//if(ev_PULSAR_BOTON == evento){
					if(ev_LEDS_SIGUINTE == evento){ // este evento es programado desde apagar leds que es llamado desde encender leds cada vez que se produce un ev_PULSAR_BOTON
						svc_alarma_activar(0,5000,ev_TIEMPO_SUPERADO,0);
						if(aux == leds_simon[posicion_pulsada]){ // Bien pulsado
								...
								estado_simon = e_FAIL;
								svc_alarma_activar(0,100,ev_JUEGO_CONTINUAR,0);
						}
					}
			//	}
		...
		case e_SUCCESS:
			if(ev_JUEGO_CONTINUAR){
				posicion_simon++;
				estado_simon = e_SHOW_SEQUENCE;
				velocidad--;
				svc_alarma_activar(0, 10, ev_LEDS_ENCENDER, 0);
			}
			break;
