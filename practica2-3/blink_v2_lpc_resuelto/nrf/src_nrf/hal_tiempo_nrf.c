...
#define MAX_COUNTER_VALUE 0xFFFFFFFF			// Maximo valor del contador de 32 bits
#define HAL_TICKS2US_T0	  16 					// frecuencia en MHz a la que funciona nrf52840 TIMER0 
#define HAL_TICKS2US_T1	  16 					// frecuencia en MHz a la que funciona nrf52840 TIMER1
...
void TIMER0_IRQHandler(void) {
    ...
    if (NRF_TIMER0->EVENTS_COMPARE[0]) {    
        NRF_TIMER0->EVENTS_COMPARE[0] = 0;  // Limpiar la interrupción
				// Variable para asegurarse que hemos limpiado la interrupción
...
uint32_t hal_tiempo_iniciar_tick() {
    ...
    return HAL_TICKS2US_T0;   // Devuelve el factor de conversión de ticks a microsegundos
}
...
uint64_t hal_tiempo_actual_tick() { 
    ...
    time = ((uint64_t)MAX_COUNTER_VALUE * (uint64_t)timer0_int_count) + (uint64_t)NRF_TIMER0->CC[0]; 
		return time;
}
