...
void drv_botones_tratar(EVENTO_T evento, uint32_t auxiliar){
	...
	case e_reposo:
		if (evento == ev_BOTON_RETARDO) {
			...
			uint32_t id_boton = (uint32_t)boton_equivalente_pin(auxiliar);
			...
			f_callback_drv(ev_PULSAR_BOTON, id_boton);

			// Activar una alarma esporádica (Trp) para 5 ms
			svc_alarma_activar(svc_alarma_codificar(0, TRP), ev_BOTON_RETARDO, id_boton);
	...
	case e_entrando: 
		if (evento == ev_BOTON_RETARDO) {
			...
			svc_alarma_activar(svc_alarma_codificar(0, TEP), ev_BOTON_RETARDO, id_boton);
