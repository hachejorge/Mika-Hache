...
void test_WDT(){
		hal_WDT_iniciar(10);
		drv_led_encender(1);
		while(1);				// se queda en un bucle infinito y deber�a salir de este al reiniciarse a los 10 segundos
}

void test_rt_FIFO(){
	rt_FIFO_inicializar(1);
	//verificar_insertar_extraer_en_cola();
	verificar_overflow_cola(); // inserta m�s eventos que tama�o de la cola para que se produzca un overflow
}
...
void test_alarmas(){
	svc_alarma_iniciar(1,rt_FIFO_encolar,ev_T_PERIODICO); // inciamos la alarma peri�dica del ev_T_PERIODICO
