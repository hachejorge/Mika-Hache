	...
	switch(estado_simon) 
	{
				...
				if(ev_PULSAR_BOTON == evento){
						if(aux == leds_simon[posicion_pulsada]){ // Bien pulsado
								...
								if(posicion_simon == posicion_pulsada + 1){ // Ha pulsado todos bien
										...
										svc_alarma_activar(svc_alarma_codificar(0, 1000), ev_JUEGO_CONTINUAR, 0);
								}
								else{
										posicion_pulsada++;
								}
		...
		case e_SUCCESS:
			if(evento == ev_JUEGO_CONTINUAR){
				...
				estado_simon = e_SHOW_SEQUENCE;
				velocidad--;
				svc_alarma_activar(svc_alarma_codificar(0, 1000), ev_LEDS_ENCENDER, 0);
			}
			break;
		...
		case e_FAIL:
			if(evento == ev_JUEGO_CONTINUAR){
				posicion_simon = 1;
				estado_simon = e_INIT;
				svc_alarma_activar(svc_alarma_codificar(0, 1000), ev_INICIAR_JUEGO, 0);
			}
			break;
