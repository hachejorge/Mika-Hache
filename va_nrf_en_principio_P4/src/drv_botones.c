...
#define TRP 5 // Tiempo de rebote de entrada
#define TEP 5 // Periodo para comprobar si se ha soltado el botón
#define TRD 5 // Tiempo de rebote de salida
...
bool boton_pulsado(uint32_t pin) {
    return hal_gpio_leer(pin) != BUTTONS_ACTIVE_STATE;
}
...
void drv_botones_iniciar(void(*callback)(EVENTO_T id, uint32_t aux), EVENTO_T ev_PULSAR_BOTON, EVENTO_T ev_BOTON_RETARD0){
		...
		for (uint32_t i = 0; i < BUTTONS_NUMBER; ++i) 			{
			...
			hal_habilitar_despertar(button_list[i]);
		}
...
void drv_botones_tratar(EVENTO_T evento, uint32_t auxiliar){
	...
	case e_reposo:
		if (evento == ev_PULSAR_BOTON) {
			...
			svc_alarma_activar(false, TRP, ev_BOTON_RETARDO, auxiliar);
	...
	case e_entrando: 
		if (evento == ev_BOTON_RETARDO) {			
			...
			svc_alarma_activar(true, TEP, ev_BOTON_RETARDO, auxiliar);
	...
	case e_esperando:
		...
		if (evento == ev_BOTON_RETARDO) {
			...
			if (!boton_pulsado(button_list[auxiliar-1])){
				...
				svc_alarma_activar(false, TRD, ev_BOTON_RETARDO, auxiliar);
