...
void test_alarmas(){
	...
	svc_alarma_activar(1,1000, ev_PULSAR_BOTON, 10);
	EVENTO_T evento;
  uint32_t auxiliar;
...
void juego(EVENTO_T evento, uint32_t aux){
	...
	case juego_activo:
		if(aux == leds_juego[pos_led]){
			...
			svc_alarma_activar(false, 2, ev_INICIAR_JUEGO, 0);
		}
		break;
...
#define NUM_LEDS_SIMON 8
static int leds_simon[NUM_LEDS_SIMON];

void pruebas_random(){
	hal_random_iniciar(drv_tiempo_actual_ms());
	for(int i = 0; i < NUM_LEDS_SIMON; i++){
		leds_simon[i] = hal_random_generar(1, BUTTONS_NUMBER);
	}
	int j = 0;
	while( j < NUM_LEDS_SIMON){
		drv_led_conmutar(leds_simon[j]);
		j++;
	}
}
